...
func _init() -> void:
	...
	# Make PotatoRadio folder if it doesn't exist
	var dir := Directory.new()
	if not dir.dir_exists("user://PotatoRadio"):
		dir.make_dir("user://PotatoRadio")
	...
	if not File.new().file_exists("user://PotatoRadio/%s" % executable_name):
		_downloader.download(yt_dlp_sources[OS.get_name()], "user://PotatoRadio/%s" % executable_name)
	...
	if OS.get_name() == "Windows":
		...
		OS.execute("chmod", PoolStringArray(["+x", OS.get_user_data_dir() + "/PotatoRadio/yt-dlp"]))
...
func _setup_ffmpeg() -> void:
	...
	if not file.file_exists("user://PotatoRadio/ffmpeg.exe"):
		_downloader.download(ffmpeg_sources["ffmpeg"], "user://PotatoRadio/ffmpeg.exe")
		yield(_downloader, "download_completed")
	
	if not file.file_exists("user://PotatoRadio/ffprobe.exe"):
		_downloader.download(ffmpeg_sources["ffprobe"], "user://PotatoRadio/ffprobe.exe")
		yield(_downloader, "download_completed")


func _update_yt_dlp(arguments: Array) -> void:
	OS.execute("%s/PotatoRadio/%s" % [OS.get_user_data_dir(), arguments[0]], ["--update"])
...
func _execute_on_thread(arguments: Array) -> void:
	...
	var executable: String = OS.get_user_data_dir() + "PotatoRadio/" + \
